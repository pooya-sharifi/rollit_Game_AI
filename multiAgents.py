...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, state):
        ...
        # Initialize variables to store the best move and its corresponding score
        bestMove = None
        bestScore = float('-inf')  # Initialize to negative infinity for maximization

        for action in legalMoves:
            print("number of moves we do")
            # For each legal move, get the successor state
            successorState = state.generateSuccessor(self.index, action)

            print(successorState)

            # print(self.index)
            score = self.minimax(successorState, self.depth, (self.index+1) % state.getNumAgents())
    ...
    def minimax(self, state, depth, currentPlayerIndex):
        # Base case: If depth is 0 or game is finished, evaluate the state
        print("#####index we looking for :",currentPlayerIndex)
        print("dept")
        if depth == 0 or state.isGameFinished():
            print("minmax ended")
            return self.evaluationFunction(state)

        legalMoves = state.getLegalActions(currentPlayerIndex)

        print("Legal moves",legalMoves)

        if currentPlayerIndex == 0:
            print("Maximizing player's turn (your agent)")
            maxScore = float('-inf')
            ...
            for action in legalMoves:
                ...
                print("$$$_1")
                print("Next state we pass to minimax",currentPlayerIndex+1)

                score = self.minimax(successorState, depth - 1, (currentPlayerIndex+1) % state.getNumAgents())
                print(score)
                maxScore = max(maxScore, score)
            return maxScore
        else:

            print("Minimizing player's turn (opponent)")
            print(legalMoves)

            minScore = float('inf')
            for action in legalMoves:
                successorState = state.generateSuccessor(self.index, action)
                print(currentPlayerIndex+1)
                print("$$$_2")
                print("Next state we pass to minimax",(currentPlayerIndex+1)% state.getNumAgents())
